...
namespace EncryptNote
{
    ...
    /// </remarks>
    class Base85Converter : IBytearrayToTextConverter
    {
        /// <summary>
        ...
        public byte[] Decode(string encodedString)
        {
            if (EnforceMarks)
            {
                if (!encodedString.StartsWith(PrefixMark) | !encodedString.EndsWith(SuffixMark))
                {
        ...
        {
            StringBuilder sb = new StringBuilder((int)(byteArray.Length * (_encodedBlock.Length / _decodedBlock.Length)));
            _linePos = 0;
